...
describe("Interact with Inputs", () => {
  ...
  it("Enter your full Name", () => {
    cy.get("#fullName").type("Jeevan Paudel");
    //Asserting
    cy.get("#fullName").should("have.value", "Jeevan Paudel");
  });
  it("Append a text and press keyboard tab", () => {
    cy.get("#join").type(", thank you");
    //Asserting
    cy.get("#join").should("have.value", "I am good, thank you");
    ...
    cy.get("#join").tab(); //A Cypress plugin to add a tab command
    cy.get("#getMe").should("be.focused");
  });
